...
def run(
    ...
    stream: bool = typer.Option(
        True, "--stream/--no-stream", help="Stream the response (enabled by default)"
    ),
    ...
    execute: bool = typer.Option(
        ...
        "--execute/--no-execute",
        "-e",
        help="Execute the prompt via API (enabled by default)",
    ),
    ...
    prompt = None
    if current_collection:
        prompt = manager.collection_storage.get_xml_collection_template(
    ...
    if commands_enabled:
        ...
        generated_prompt, _ = prompt.render(
...
def collection_create(
    ...
    system_prompt: str = typer.Option(
        ...
        "--system-prompt",
        "-s",
        help='System prompt as JSON string (e.g., \'{"role": "system", "content": "You are..."}\')',
    ),
):
    ...
    success = manager.create_collection(
        name, description, templates, tags, system_prompt
    )
    if success:
        console.print(
...
def collection_import(
    path: Path = typer.Argument(..., help="Path to collection XML file to import"),
...
def provider_add(
    name: Optional[str] = typer.Argument(
        None, help="Provider name (optional - will prompt if not provided)"
    ),
    base_url: Optional[str] = typer.Argument(
        None, help="Base URL for the API (optional - will prompt if not provided)"
    ),
    model: Optional[str] = typer.Option(
        None, "--model", "-m", help="Default model for this provider"
    ),
    header: Optional[List[str]] = typer.Option(
        None, "--header", "-h", help="Custom headers (format: 'Header:Value')"
    ),
    auth_type: Optional[str] = typer.Option(
        "bearer", "--auth-type", help="Authentication type (bearer, api-key, none)"
    ),
    quick: bool = typer.Option(
        False, "--quick", "-q", help="Skip wizard and use minimal setup"
    ),
):
    ...
    try:
        ...
        if name and base_url:
            ...
            if success:
                console.print(
                    f"Custom provider '{name}' added successfully!", style="green"
                )
                console.print(
                    f"Use with: aix run <prompt> --provider custom:{name}", style="cyan"
                )
            else:
        ...
        console.print(
            "This wizard will help you configure a custom API provider step by step.\n"
        )
        ...
        if not name:
            ...
            console.print(
                "Choose a unique name to identify this provider (e.g., 'ollama', 'local-api')"
            )
        ...
        if not base_url:
            ...
            console.print(
                "Enter the base URL for your API endpoint (must end with /v1 for OpenAI compatibility)"
            )
            ...
            if not base_url.startswith(("http://", "https://")):
                console.print(
                    "⚠️  URL should start with http:// or https://", style="yellow"
                )
            if not base_url.endswith("/v1"):
                console.print(
                    "⚠️  For OpenAI compatibility, URL should end with /v1",
                    style="yellow",
                )
            console.print()
        ...
        console.print(
            "Specify a default model name for this provider (can be left empty)"
        )
        ...
        if needs_auth:
            ...
            if auth_choice in [2, 3]:
                ...
                while True:
                    header_input = typer.prompt(
                        "Header (format: Key:Value)", default="", show_default=False
                    )
        ...
        console.print(
            f"│ API Key: [cyan]{'Configured' if api_key else 'Not set'}[/cyan]"
        )
        ...
        success = config.add_custom_provider(
            name=name, base_url=base_url, default_model=default_model, headers=headers
        )
...
def provider_list():
    ...
    try:
        ...
        if not providers:
            ...
            console.print(
                "Add one with: aix provider add <name> <base-url>", style="cyan"
            )
            return
        ...
        for name, provider_data in providers.items():
            console.print(
                f"  {name}: {provider_data.get('base_url', 'N/A')}", style="green"
            )
            if provider_data.get("default_model"):
                console.print(
                    f"    Default model: {provider_data['default_model']}", style="dim"
                )
            console.print(
                f"    Auth Type: {provider_data.get('auth_type', 'bearer')}",
                style="dim",
            )
...
def provider_remove(name: str = typer.Argument(..., help="Provider name")):
    ...
    try:
        ...
        if success:
            console.print(
                f"Custom provider '{name}' removed successfully!", style="green"
            )
        else:
...
def provider_quick_setup():
    ...
    try:
        ...
        for key, preset in presets.items():
            console.print(
                f"[bold]{key}.[/bold] [cyan]{preset['display']}[/cyan] - {preset['description']}"
            )
        ...
        # Save configuration
        success = config.add_custom_provider(
            name=name, base_url=base_url, default_model=default_model, headers={}
        )
